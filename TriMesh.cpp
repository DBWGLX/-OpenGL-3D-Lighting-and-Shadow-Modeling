...
void TriMesh::computeVertexNormals()
{
	...
	for (size_t i = 0; i < vertex_normals.size(); i++)//归一化法向量
	{
...
void TriMesh::generateCube()
{
	...
	//std::cout << "[DEBGU] cube vertex_normals.size():" << vertex_normals.size() << "\n";
...
void TriMesh::readObj(const std::string& filename)
{
	...
	std::string line;
	int current_smooth_group = 0; // 默认光滑组编号
	...
	while (std::getline(fin, line))
	{
		...
		int a2, b2, c2;
		char slash;
		_x = _y = _z = a0 = b0 = c0 = a1 = b1 = c1 = a2 = b2 = c2 = 0;
		...
		if (type == "f") {//以 'f' 开头，后跟一组顶点、法线和纹理坐标的索引，定义了模型的面。
			...
			//四边形face的兼容
			std::vector<int> a, b, c; // 用于存储所有列的索引
			while (sin >> a0 >> slash >> b0 >> slash >> c0) { // 持续读取每列的数据
				a.push_back(a0 - 1);
				b.push_back(b0 - 1);
				c.push_back(c0 - 1);

				// 检查是否还有更多列
				if (sin.peek() == ' ') sin.ignore();
				else break;
			}
			if (a.size() == 3) {
				// 如果是三角形，直接存储
				faces.push_back(vec3i(a[0], a[1], a[2]));
				texture_index.push_back(vec3i(b[0], b[1], b[2]));
				normal_index.push_back(vec3i(c[0], c[1], c[2]));
			}
			else if (a.size() == 4) {
				// 如果是四边形，拆分为两个三角形
				faces.push_back(vec3i(a[0], a[1], a[2]));
				...
				/*faces.push_back(vec3i(a[0], a[2], a[3]));
				texture_index.push_back(vec3i(b[0], b[2], b[3]));
				normal_index.push_back(vec3i(c[0], c[2], c[3]));*/

				faces.push_back(vec3i(a[0], a[3], a[2])); // 顺序调整
				texture_index.push_back(vec3i(b[0], b[3], b[2])); // 顺序调整
				normal_index.push_back(vec3i(c[0], c[3], c[2])); // 顺序调整

				//saber的.off光滑组配置
				smooth_groups.push_back(current_smooth_group);
			}
			else {
				std::cerr << "Unsupported polygon with " << a.size() << " vertices in face data.\n";
			}
		}
		...
		if (type == "s") {
			...
			sin >> group;
			if (group == "off") {
				current_smooth_group = 0; // 关闭光滑组
			}
			else {
				current_smooth_group = std::stoi(group); // 设置光滑组编号
			}
		}
	}
...
void TriMesh::reverse_vertex_normals() {
	for (auto& x : vertex_normals) {
		x = -x;
	}
}


void TriMesh::setShadowCaster(bool flag) { isShadowCaster = flag; }
bool TriMesh::getShadowCaster() const { return isShadowCaster; }
