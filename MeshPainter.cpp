...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::vector<std::string>& texture_images, const std::string &vshader, const std::string &fshader){
    ...
    GLint hasTextureMapLocation = glGetUniformLocation(object.program, "hasTextureMap");

    if (texture_images.size()) {
        object.hasTextureMap = 1;

        glUniform1i(hasTextureMapLocation, 1);//该属性决定片元着色器中是否计算纹理贴图的显示
        bindTexture(object, texture_images);
    }
    else {
        glUniform1i(hasTextureMapLocation, 0);
    }
...
void MeshPainter::bindTexture(openGLObject& object, const std::vector<std::string>& texture_images) {
...
//};
void MeshPainter::addMesh( TriMesh* mesh, const std::string &name, const std::vector<std::string> &texture_images, const std::string &vshader, const std::string &fshader ){
	mesh_names.push_back(name);
    meshes.push_back(mesh);

    std::cout << "[Loading]: " << name << " texture_images num:" << texture_images.size() << "\n";

    openGLObject object;
    // 绑定openGL对象，并传递顶点属性的数据
    bindObjectAndData(mesh, object, texture_images, vshader, fshader);
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
    ...
    //影子
    if (mesh->getShadowCaster()) {
        // 定义光源位置（点光源）和平面方程的参数。
        glm::vec3 lightPosition = glm::vec3(light->getTranslation()); // 光源位置
        glm::vec3 planeNormal = glm::vec3(0.0f, 1.0f, 0.0f);          // 平面法向量
        float planeD = 0.0f; // 平面方程 Ax + By + Cz + D = 0 中的 D

        // 构造阴影矩阵
        glm::mat4 shadowMatrix(1.0f);
        float dot = glm::dot(planeNormal, lightPosition) + planeD;

        shadowMatrix[0][0] = dot - lightPosition.x * planeNormal.x;
        shadowMatrix[1][0] = -lightPosition.x * planeNormal.y;
        shadowMatrix[2][0] = -lightPosition.x * planeNormal.z;
        shadowMatrix[3][0] = -lightPosition.x * planeD;

        shadowMatrix[0][1] = -lightPosition.y * planeNormal.x;
        shadowMatrix[1][1] = dot - lightPosition.y * planeNormal.y;
        shadowMatrix[2][1] = -lightPosition.y * planeNormal.z;
        shadowMatrix[3][1] = -lightPosition.y * planeD;

        shadowMatrix[0][2] = -lightPosition.z * planeNormal.x;
        shadowMatrix[1][2] = -lightPosition.z * planeNormal.y;
        shadowMatrix[2][2] = dot - lightPosition.z * planeNormal.z;
        shadowMatrix[3][2] = -lightPosition.z * planeD;

        shadowMatrix[0][3] = -planeNormal.x;
        shadowMatrix[1][3] = -planeNormal.y;
        shadowMatrix[2][3] = -planeNormal.z;
